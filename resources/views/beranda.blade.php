...
<body>
            ...
            <nav class="hidden md:flex gap-12 text-[10px] uppercase tracking-[0.4em] font-light items-center">
                ...
                @if (Auth::check())
                    <div class="flex items-center gap-5 ml-6 border-l border-white/5 pl-8">
                        <span class="text-white/60">HI, {{ strtoupper(Auth::user()->name) }}</span>
    ...
    <div class="action-section">
        ...
        <div class="action-card">
            ...
            @else
                <a href="{{ route('login') }}" class="btn-book-action">
                    Login untuk Booking
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
        ...
        <div class="action-card">
            ...
            <div class="action-card-divider"></div>
            <form action="{{ route('booking.track') }}" method="POST">
                @csrf
    ...
    {{-- ═══════════════════════════════════════════ --}}
    {{--  BOOKING STATUS CARD (if logged in)        --}}
    {{-- ═══════════════════════════════════════════ --}}
    @if (Auth::check() && isset($booking))
                ...
                <div class="status-card-inner">
                    ...
                    <div class="status-header">
                        <div class="status-header-left">
                            ...
                            <h2 class="status-title">{{ ucfirst($booking->kategori) }} &bull;
                                {{ $booking->photographer_name ?? 'Assigning Soon...' }}</h2>
                            <p class="status-subtitle">{{ date('d F Y', strtotime($booking->tanggal_pemotretan)) }}
                            </p>
                        </div>
                        <div>
                    ...
                    <div class="status-main-grid">
                        <div style="display:flex; flex-direction:column; gap:20px;">
                            ...
                            @if ($displayPaymentStatus || $displayAmountPaid)
                                <div class="payment-status-section">
                                    <div class="payment-info-box">
                                        <div class="payment-label">Status Pembayaran</div>
                                        <div class="payment-value">
                                            {{ strtoupper($displayPaymentStatus ?? 'PENDING') }}</div>
                                        <div class="payment-subtext">
                                            @if ($displayPaymentStatus === 'full_payment')
                                                ✓ Pembayaran Lengkap
                                            @elseif ($displayPaymentStatus === 'down_payment')
                                                ⚠ DP Diterima
                                            @else
                                                ⏳ Menunggu Pembayaran
                                            @endif
                                    ...
                                    <div class="payment-info-box">
                                        ...
                                        <div class="payment-value">Rp
                                            {{ number_format($displayAmountPaid ?? 0, 0, ',', '.') }}</div>
                                        <div class="payment-subtext">dari Rp
                                            {{ number_format($booking->harga, 0, ',', '.') }}</div>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if ($booking->status === 'Rejected' && $booking->rejection_note)
                                <div class="rejection-alert">
                                    ...
                                    <div class="rejection-alert-text">{{ $booking->rejection_note }}</div>
                                </div>
                            @endif
                        ...
                        <div style="display:flex; flex-direction:column; gap:12px; min-width:200px;">
                            <div
                                ...
                                <div
                                    ...
                                    Booking ID</div>
                                <div style="font-size:1rem; font-weight:700; color:#fff;">#{{ $booking->id }}</div>
                            </div>
                            <div
                                style="background:rgba(255,255,255,0.025); border:1px solid rgba(255,255,255,0.05); border-radius:14px; padding:18px 20px;">
                                <div
                            ...
                            @if ($booking->link_results)
                                <a href="{{ $booking->link_results }}" target="_blank" class="upload-proof-cta">
                            ...
                            @if (!$booking->proof_payment)
                                <form action="{{ route('user.upload.proof', $booking->id) }}" method="POST"
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const targetDate =
                    "{{ \Carbon\Carbon::parse($booking->tanggal_pemotretan)->format('Y-m-d') }}T00:00:00";
    ...
    <section class="py-40 px-10 md:px-20 max-w-[1500px] mx-auto reveal" id="services">
        ...
        <div class="gallery-grid" style="animation-delay: 0.4s;">
            ...
            @foreach ($categories as $cat)
                <a href="/portfolio/{{ $cat['slug'] }}" class="gallery-item group">
                    <img src="{{ asset('images/' . $cat['img']) }}" alt="{{ $cat['title'] }}">
                    <div class="item-text">
                        <p class="text-[9px] uppercase tracking-[0.4em] text-[#E1C564] mb-3">{{ $cat['sub'] }}</p>
                        <h3 class="playfair text-3xl text-white font-bold tracking-tight">{{ $cat['title'] }}</h3>
                    </div>
                </a>
            @endforeach
        </div>
    ...
    <section class="bpkg-section reveal" id="paket">
        ...
        <div class="bpkg-grid">
            @foreach ($packages as $pkg)
                ...
                @endphp
                <div class="bpkg-card {{ $pkg->is_popular ? 'bpkg-popular' : '' }}">
                    @if ($pkg->is_popular)
                        <div class="bpkg-popular-badge">✦ TERPOPULER</div>
                    @endif
                    <div class="bpkg-kategori">{{ strtoupper($pkg->kategori ?? 'Studio') }}</div>
                    <div class="bpkg-name">{{ $pkg->nama_paket }}</div>
                    <div class="bpkg-desc">{{ $pkg->deskripsi ?? '' }}</div>
                    <div class="bpkg-price">Rp {{ $total }}</div>
                    <div class="bpkg-dp">DP <span>Rp {{ $dp }}</span> (30%) &nbsp;·&nbsp; Total <span>Rp
                            {{ $total }}</span></div>
                    ...
                    @auth
                        ...
                        @else
                            <a href="/booking/{{ $pkg->slug }}" class="bpkg-btn bpkg-btn-gold">
                                › PESAN SEKARANG
                            </a>
                        @endif
                    @else
    ...
    <section id="about" class="py-40 bg-[#080808] border-t border-white/5 reveal">
        <div class="max-w-7xl mx-auto px-10 md:px-20 flex flex-col md:flex-row items-center gap-24">
            <div class="w-full md:w-1/2 relative group">
                ...
                <img src="{{ asset('images/foto1.jpeg') }}" alt="About"
    ...
    <section class="py-28 px-6 md:px-10 bg-black reveal" id="testimonials">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto" id="testiGrid">
            @foreach ($testimonials as $index => $t)
                <div class="testi-card {{ $index >= 3 ? 'testi-hidden' : '' }}"
                    style="{{ $index >= 3 ? 'display:none;' : '' }}">

                    {{-- Top row: avatar + name + stars --}}
                    <div class="flex items-center gap-3 mb-5">
                        <div class="testi-avatar">{{ substr($t->nama_customer, 0, 1) }}</div>
                        <div class="min-w-0">
                            <strong
                                class="text-white block text-[10px] tracking-[0.2em] uppercase truncate leading-tight">{{ $t->nama_customer }}</strong>
                            <div class="flex gap-0.5 mt-1.5">
                                @for ($i = 1; $i <= 5; $i++)
                                    <span class="testi-star {{ $i <= $t->bintang ? 'lit' : '' }}">★</span>
                                @endfor
                            </div>
                        </div>
                    </div>
                    ...
                    @if (!empty($t->admin_reply))
                        <div class="studio-reply mt-auto">
                            ...
                            <span class="studio-reply-text">{{ $t->admin_reply }}</span>
                        </div>
                    @endif
        ...
        @if ($testimonials->count() > 3)
            <div class="text-center mt-12">
                <button onclick="toggleTestimonials()" id="testiBtn" class="testi-more-btn">
                    ...
                    <span style="opacity:.35;font-size:9px;letter-spacing:.15em;">({{ $testimonials->count() - 3 }}
                        lainnya)</span>
                </button>
            </div>
        @endif
    ...
    <p class="text-[9px] text-gray-700 tracking-[0.6em] uppercase">&copy; {{ date('Y') }} LUXELENS STUDIO.
