<div id="luxelens-booking-table" wire:key="{{ $componentRootKey }}" x-data="{
    payModal: false,
    payId: null,
    payTotal: 0,
    payPaid: 0,
            ...
            <tbody>
                @forelse ($bookings as $b)
                    ...
                    <tr wire:key="booking-row-{{ $b->id }}"
                        ...
                        <td style="padding:14px 12px;color:#333;font-weight:700;">#{{ $b->id }}</td>

                        <td style="padding:14px 12px;">
                            <div style="font-size:13px;font-weight:600;color:#fff;">{{ $b->nama_customer }}</div>
                            <div style="font-size:10px;color:#444;margin-top:2px;">{{ $b->nomor_telepon }}</div>
                        </td>

                        <td style="padding:14px 12px;">
                            <span
                                style="font-size:10px;letter-spacing:.1em;font-weight:600;color:#aaa;text-transform:uppercase;">{{ $b->kategori }}</span>
                        </td>

                        <td style="padding:14px 12px;font-size:11px;color:#666;">
                            {{ date('d M Y', strtotime($b->tanggal_pemotretan)) }}
                        </td>
                            ...
                            <form action="{{ route('admin.update.photographer', $b->id) }}" method="POST">
                                ...
                                <select name="photographer_name" onchange="this.form.submit()"
                                    ...
                                    @foreach ($photographers as $p)
                                        @php $pn = $p->name.' ('.$p->team_name.')'; @endphp
                                        <option value="{{ $pn }}"
                                            {{ $b->photographer_name === $pn ? 'selected' : '' }}>{{ $pn }}
                                        </option>
                                    @endforeach
                                </select>
                            </form>
                        ...
                        <td style="padding:14px 12px;">
                            ...
                            @elseif ($b->status === 'Pending Verification' || $b->status === 'Confirmed' || $b->status === 'Lunas')
                                <span
                            ...
                            @elseif ($b->status === 'Waiting for Quote')
                                <span
                            ...
                            @else
                                <span
                                    style="padding:4px 10px;border-radius:20px;background:rgba(243,156,18,.1);color:#f39c12;border:1px solid rgba(243,156,18,.2);font-size:9px;font-weight:800;letter-spacing:.1em;">{{ strtoupper($b->status) }}</span>
                            @endif
                            @if ($b->proof_payment)
                                <a href="{{ asset('storage/proofs/' . $b->proof_payment) }}" target="_blank"
                                    ...
                                    VIEW PROOF</a>
                            @endif
                            @if ($b->booking_type === 'event' && $b->event_details)
                                <div style="margin-top:5px;font-size:8px;color:#555;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                                    title="{{ $b->event_details }}">
                                    📋 {{ Str::limit($b->event_details, 30) }}
                                </div>
                            @endif
                        </td>
                        ...
                        <td style="padding:14px 12px;">
                            <div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;">
                                <a href="/admin/download-pdf/{{ $b->id }}"
                                ...
                                @if (!in_array($b->status, ['Cancelled', 'Rejected'], true))
                                    @if ($isDone)
                                        <span
                                            ...
                                            DONE</span>
                                        @if ($hasResultLink)
                                            <a href="{{ $b->link_results }}" target="_blank"
                                    ...
                                    @elseif ($isEditingStage)
                                        <button type="button"
                                            @click="openSelesai({{ $b->id }}, '{{ $b->link_results ?? '' }}')"
                                            ...
                                            GDRIVE</button>
                                    @elseif ($canValidatePayment)
                                        <button type="button"
                                            @click="openPay({{ $b->id }}, {{ $b->harga }}, {{ $b->amount_paid ?? $b->harga }}, '{{ $b->payment_status ?? 'full_payment' }}')"
                                            ...
                                            PEMBAYARAN</button>
                                        <button type="button" @click="openReject({{ $b->id }})"
    ...
    @if ($bookings->hasPages())
        <div style="margin-top:28px;padding-top:20px;border-top:1px solid rgba(255,255,255,.05);">
            <div style="text-align:center;margin-bottom:14px;font-size:11px;color:#444;letter-spacing:.08em;">
                Halaman <strong style="color:#E1C564;">{{ $bookings->currentPage() }}</strong>
                dari <strong style="color:#fff;">{{ $bookings->lastPage() }}</strong>
                · Total <strong style="color:#fff;">{{ $bookings->total() }}</strong> booking
            </div>
    ...
    {{-- ══════════════════════════════════════════
         PAYMENT VALIDATION MODAL — Luxe Dark Gold
    ══════════════════════════════════════════ --}}
    <style>
        /* Pay Modal Core */
        .pm-overlay {
    ...
    <div x-show="payModal" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0"
        ...
        <div class="pm-card pm-card-anim" x-show="payModal" x-transition:enter="transition ease-out duration-280"
            ...
            <form :action="'/admin/bookings/' + payId + '/verify-payment'" method="POST" id="pmForm"
                ...
                {{-- Hidden select keeps actual POST value --}}
