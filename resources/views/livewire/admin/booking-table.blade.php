                            ...
                            <div style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;">
                                ...
                                @if ($b->status !== 'Cancelled')
                                    @if ($b->status !== 'Lunas')
                                        @if ($b->proof_payment)
                                            <button type="button"
                                                @click="openPay({{ $b->id }}, {{ $b->harga }}, {{ $b->amount_paid ?? $b->harga }}, '{{ $b->payment_status ?? 'pending' }}', '{{ $b->link_results ?? '' }}')"
                                    ...
                                    @elseif ($b->status === 'Lunas')
                                        <a href="/admin/update-status/{{ $b->id }}/Editing"
